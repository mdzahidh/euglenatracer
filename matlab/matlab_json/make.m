...
if ispc
    ...
    json_c_path = 'c:\src\euglenalab\game\detection\matlab\json-c'
    ...
    mexargs_json = [{
        ...
        ['LINKFLAGS=$LINKFLAGS', '/NODEFAULTLIB:MSVCRT '], ...
        } mexargs_json];
    
%      '/NODEFAULTLIB:MSVCRTD ', '/NODEFAULTLIB:LIBCMT ', '/NODEFAULTLIB:LIBCMTD'
...
elseif isunix
    ...
    mexargs_json = ['-DJSON_C_DIR_PREFIXED ', mexargs_json]; 
    mex('-v', '-Dinline=__inline', '-ljson-c','-L../../local/lib','-I../../local/include/json-c','fromjson.c')
    mex('-v', '-Dinline=__inline', '-ljson-c','-L../../local/lib','-I../../local/include/json-c','-lm', 'tojson.c')
end

disp '****** MEXXXXX', mexargs_common{:}
