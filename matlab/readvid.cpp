...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if( nrhs < 1 ){
        ...
        cerr << "\tUsage: readvid( movieFileName)" << endl;
        return;
    }
    ...
    mwSize dims[] = {height,width,3, totalFrames};
    mxArray* output = mxCreateNumericArray(4,dims, mxUINT8_CLASS,mxREAL);
    uchar* pdata = (uchar*)mxGetData(output);
    unsigned int frameSize = width * height * 3;
    unsigned int channelSize = width * height;
    ...
    while(1){
        ...
        for(int ch=0;ch<3;ch++){
            uchar* pPlane = pdata + ch * channelSize  +  frameN * frameSize;
            for(int c=0;c<width;c++){
                for(int r=0;r<height;r++){
                    *pPlane++ = channel[2-ch].at<uchar>(r,c);
                }
            }
        }
        frameN ++;
        progressBar( frameN / (float)totalFrames );
    }
